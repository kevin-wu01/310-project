...
function attemptDirRead(currentPath) {
    try {
        ...
        Log_1.default.error("Error reading directory " + currentPath);
        throw err;
    }
}
...
function readAllFiles(currentPath) {
    ...
    try {
        for (var filesInDir_1 = __values(filesInDir), filesInDir_1_1 = filesInDir_1.next(); !filesInDir_1_1.done; filesInDir_1_1 = filesInDir_1.next()) {
            ...
            var fullPath = currentPath + "/" + fileOrDirName;
...
function readTestsFromDisk(path) {
    ...
    try {
        ...
        if (files.length === 0) {
            Log_1.default.warn(methodName + " No folder-test files found in " + path + ".");
        }
    }
    catch (err) {
        Log_1.default.error(methodName + " Exception reading files in " + path + ".");
        throw err;
    }
    try {
        for (var files_1 = __values(files), files_1_1 = files_1.next(); !files_1_1.done; files_1_1 = files_1.next()) {
            ...
            try {
                ...
                Log_1.default.error(methodName + " Could not read " + file + ".");
                throw err;
            }
            try {
                var test_1 = JSON.parse(content.toString());
                test_1.filename = file.replace(path + "/", ""); // dont show absolute paths
                testsLoaded.push(test_1);
            }
            catch (err) {
                Log_1.default.error(methodName + " " + file + " could not be read and parsed.");
